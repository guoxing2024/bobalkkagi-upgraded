...
def saveDumpfile(fileName:str, data:bytes) -> None:
    ...
    if not os.path.isdir('dumpfiles'):
        os.mkdir('dumpfiles')
    baseName = os.path.splitext(os.path.basename(fileName))[0] + '.dump'
    if os.path.isfile(os.path.join('dumpfiles', baseName)):
        baseName = os.path.splitext(baseName)[0] + '_new.dump'
    path = os.path.join('dumpfiles', baseName)
    f =open(path,'wb')
    f.write(data)
    f.close()
