...
def PEinfo(func, variable):
    ...
    imagebase = GLOBAL_VAR.image_base
...
def Injection_call(new_virtualAddress):
    ...
    for call_offset in call_addrfunc: # call_offset 
        ...
        #e8
            ...
            if(GLOBAL_VAR.image_base <= operand_virtualaddr and operand_virtualaddr <= GLOBAL_VAR.image_end):
...
def hooking_code(uc, address, size, user_data):
    ...
    if not (GLOBAL_VAR.themida_section[1] <= address and address < (GLOBAL_VAR.themida_section[1]+GLOBAL_VAR.themida_section[2])):
        count += 1
    elif count >= 1 and GLOBAL_VAR.themida_section[1] <= address and address < (GLOBAL_VAR.themida_section[1]+GLOBAL_VAR.themida_section[2]):
        count = 0
    if count >=4:
        count = 0
        uc.emu_stop()
...
def call_emulate_rip(uc, rip):
    ...
    try:
        ...
        uc.emu_start(GLOBAL_VAR.image_base + rip, GLOBAL_VAR.image_base +rip+ 0x20000)
        uc.hook_del(hookint2)
        uc.hook_del(hookint)
    except UcError as e:
        uc.hook_del(hookint)
...
def check_jmp_Instruction(rip):
    ...
    try:
        offset = readDword(rip + 3)
        address = readLword(offset + rip+ 7)
        if GLOBAL_VAR.themida_section[1] <= address and address < (GLOBAL_VAR.themida_section[1]+GLOBAL_VAR.themida_section[2]):
...
def pattern_target():
    assem = Decode(GLOBAL_VAR.text_section[0], origin_data[GLOBAL_VAR.text_section[0]:GLOBAL_VAR.text_section[0]+GLOBAL_VAR.text_section[1]], Decode64Bits)
...
def unwrapping(dump, OEP:int):
    ...
    if not PEcompare():
        ...
        dumpFileName = 'unwrap_'+GLOBAL_VAR.sample_path.split('\\')[-1].split('.')[0] + '.dump'
